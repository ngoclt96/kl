                    ...
                    <div class="x_content">
                        <div id="datatable-responsive_wrapper"
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-8">
                                    <a class="btn btn-info pull-right btn-sm" aria-label="Left Align" href="{{ route('courses.form') }}">New Courses</a>
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    <table id="datatable" class="table table-striped table-bordered dataTable no-footer"
                                        ...
                                        @else
                                            @foreach ($pages as $course)
                                                <tr role="row" class="even">
                                                    <td class="text-center"><input type="checkbox" class="form-control check-el" name="{{ $course->id }}"></td>
                                                    @foreach($searchField as $key => $value)
                                                        @php $name = $course->$key @endphp

                                                        <td class="text-center">
                                                            @if($key == "status")
                                                                @include('pc.elements.status', ['status' => $course->status])

                                                            @elseif($key == "image")
                                                                <img src="{{$course->image }}" style="width: 50px">
                                                            @else
                                                    ...
                                                    @endforeach
                                                    <td class="text-center">
                                                        @include('pc.elements.action', ['item' => $course])
                                                    </td>
                                                </tr>
                                            @endforeach
                                        @endif
