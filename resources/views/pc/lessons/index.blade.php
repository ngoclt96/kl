                    ...
                    <div class="x_content">
                        <div id="datatable-responsive_wrapper"
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-8">
                                    <a class="btn btn-info pull-right btn-sm" aria-label="Left Align" href="{{ route('lessons.form') }}">New Lessons</a>
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    <table id="datatable" class="table table-striped table-bordered dataTable no-footer"
                                        ...
                                        @else
                                            @foreach ($pages as $lesson)
                                                <tr role="row" class="even">
                                                    <td class="text-center"><input type="checkbox" class="form-control check-el" name="{{ $lesson->id }}"></td>
                                                    @foreach($searchField as $key => $value)
                                                        @php $name = $lesson->$key @endphp

                                                        <td class="text-center">
                                                            @if($key == "status")
                                                                @include('pc.elements.status', ['status' => $lesson->status])
                                                            @else
                                                    ...
                                                    @endforeach
                                                    <td class="text-center">
                                                        @include('pc.elements.action', ['item' => $lesson])
                                                    </td>
                                                </tr>
                                            @endforeach
                                        @endif
